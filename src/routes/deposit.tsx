...
export const Route = createFileRoute("/deposit")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Deposit funds by mobile check, wire, or ACH transfer." },
    ],
  }),
...
function DepositPage() {
  ...
  const [method, setMethod] = useState<"scan" | "wire">("scan");
  const [scanState, setScanState] = useState<"idle" | "scanning" | "captured">("idle");
  const [submitted, setSubmitted] = useState<null | { amount: number; ref: string }>(null);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white border border-slate-200 rounded-xl p-8 shadow-sm text-center">
          ...
          <p className="text-sm text-slate-500 mt-1">Your funds will be available within 1 business day.</p>
          ...
          <div className="mt-6 flex gap-2">
            <button onClick={() => { setSubmitted(null); setAmount(""); setScanState("idle"); }} className="flex-1 border border-slate-300 rounded-md py-2 text-sm hover:bg-slate-50">New deposit</button>

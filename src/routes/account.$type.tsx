...
function AccountPage() {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#1a0808] flex items-center justify-center text-white">
        <div className="text-center">
          <p className="text-sm text-amber-200/80">You must sign in to view this account.</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1a0708] via-[#20090a] to-[#0e0405] text-slate-100">
      {/* Header */}
      <header className="border-b border-amber-500/25 bg-black/40 backdrop-blur">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-[0.24em] text-amber-300/90">{meta.tag}</div>
              <div className="text-sm font-semibold text-white">{meta.name}</div>
            </div>
          </div>
          <button onClick={() => navigate({ to: "/" })} className="text-xs text-amber-200/80 hover:text-amber-200 border border-amber-400/30 rounded-md px-3 py-1.5">
            ← Back to Dashboard
          </button>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        {/* Balance card */}
        <section className="rounded-2xl border border-amber-500/30 overflow-hidden shadow-2xl bg-gradient-to-br from-[#5a0d10] via-[#3b0507] to-[#1a0304] relative">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(251,191,36,0.25),transparent_55%)]" />
          <div className="relative px-8 py-9 flex flex-wrap items-start justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 text-amber-300 text-xs uppercase tracking-[0.24em] font-semibold">
                <span className="h-px w-8 bg-amber-400" />
                {meta.tag}
              </div>
              <h1 className="mt-3 text-3xl font-semibold tracking-wide text-white">{meta.name}</h1>
              <div className="text-xs text-amber-200/80 mt-1 tabular-nums">
                Account •••• {meta.acct.slice(-4)} · Routing 121000248
              </div>
            </div>
            <div className="text-right">
              <div className="text-[10px] uppercase tracking-[0.24em] text-amber-200/80">Available Balance</div>
              <div className="text-4xl font-semibold tabular-nums mt-1 bg-gradient-to-b from-amber-200 to-amber-500 bg-clip-text text-transparent">
        ...
        {/* Ledger */}
        <section className="rounded-2xl border border-amber-500/20 bg-black/40 overflow-hidden">
          <div className="px-6 py-4 border-b border-amber-500/15 flex items-center justify-between">
            <div>
              <h2 className="text-sm font-semibold text-white">{meta.name} · Activity Ledger</h2>
              <p className="text-xs text-amber-200/60 mt-0.5">{entries.length} entries · isolated to this account</p>
            </div>
            <Link to="/account/$type" params={{ type: other }} className="text-xs text-amber-300 hover:text-amber-200 border border-amber-400/30 rounded px-3 py-1.5">
              View {other === "checking" ? "Checking" : "Savings"} →
            </Link>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-white/[0.04] text-[10px] uppercase tracking-widest text-amber-200/70">
                <tr>
              ...
              <tbody>
                {entries.length === 0 && (
                  <tr><td colSpan={4} className="px-6 py-10 text-center text-slate-500 text-sm">No activity recorded for this account yet.</td></tr>
                )}
                {entries.map((e) => (
                  <tr key={e.id} className="border-t border-white/5">
                    <td className="px-6 py-3 text-slate-300 whitespace-nowrap">{new Date(e.date).toLocaleDateString()}</td>
                    <td className="px-6 py-3 text-white">{e.description}</td>
                    <td className={`px-6 py-3 text-right font-mono tabular-nums ${e.amount >= 0 ? "text-emerald-300" : "text-red-300"}`}>
                      {e.amount >= 0 ? "+" : "-"}{fmtCurrency(Math.abs(e.amount))}
                    </td>
                    <td className="px-6 py-3 text-right font-mono tabular-nums text-amber-200">{fmtCurrency(e.balanceAfter)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function InternalTransfer({ user, source }: { user: MtUser; source: AccountKey }) {
  ...
  return (
    <section className="rounded-2xl border border-amber-500/25 bg-black/40 p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="text-[10px] uppercase tracking-[0.24em] text-amber-300 font-semibold">Internal Transfer</div>
          <h3 className="text-sm font-semibold text-white">Move funds between your DBW accounts</h3>
        </div>
      </div>
      <form onSubmit={submit} className="mt-4 grid sm:grid-cols-[1fr_1fr_auto] gap-3 items-end">
        <label className="block text-xs uppercase tracking-wider text-amber-200/70">
          From
          <div className="mt-1 rounded-md border border-amber-400/25 bg-black/40 px-3 py-2 text-sm text-white">{srcLabel}</div>
        </label>
        <label className="block text-xs uppercase tracking-wider text-amber-200/70">
          Destination
          <select value={dest} onChange={(e) => setDest(e.target.value as AccountKey)}
            className="mt-1 w-full rounded-md border border-amber-400/25 bg-black/40 px-3 py-2 text-sm text-white focus:outline-none focus:border-amber-400">
        ...
        <label className="block text-xs uppercase tracking-wider text-amber-200/70">
          ...
          <input type="number" step="0.01" min="0" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00"
            className="mt-1 w-full rounded-md border border-amber-400/25 bg-black/40 px-3 py-2 text-sm text-white font-mono tabular-nums focus:outline-none focus:border-amber-400" />
        </label>
        <div className="sm:col-span-3 flex items-center justify-between gap-3">
          {msg
            ? <div className={`text-xs rounded px-3 py-2 border ${msg.ok ? "border-emerald-400/40 bg-emerald-500/10 text-emerald-300" : "border-red-400/40 bg-red-500/10 text-red-300"}`}>{msg.text}</div>
            : <div className="text-[11px] text-amber-200/60">Transfers post instantly and log to both account ledgers.</div>}
          <button type="submit" className="rounded-md bg-gradient-to-r from-amber-300 to-amber-600 text-black text-xs font-bold uppercase tracking-widest px-5 py-2.5 hover:brightness-110">
            Submit Transfer
          </button>
        </div>
      </form>
